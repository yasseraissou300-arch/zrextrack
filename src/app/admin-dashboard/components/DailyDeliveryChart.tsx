...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 rounded-xl shadow-lg p-3 min-w-[150px]">
      <p className="text-xs font-semibold text-stone-400 dark:text-stone-500 mb-2 uppercase tracking-wide">
        {label}
      </p>
...
export default function DailyDeliveryChart() {
  ...
  const fetch_ = async () => {
    try {
      ...
      /* graphique non bloquant : on garde les données précédentes */
    }
    setLoading(false);
  };
    ...
    <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3 text-xs text-stone-400 dark:text-stone-500">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-sm bg-stone-300" />
            Retours
          </span>
        </div>
      </div>
      {loading ? (
        <div className="h-[220px] flex items-center justify-center text-stone-300 dark:text-stone-600 text-sm">
          Chargement...
        </div>
      ) : (
