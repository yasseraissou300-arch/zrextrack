  ...
  return (
    <div className="bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 rounded-xl shadow-lg p-3">
      ...
      <p className="text-xs text-stone-400 dark:text-stone-500">
        {total > 0 ? ((item.value / total) * 100).toFixed(1) : 0}% du total
      </p>
    </div>
  );
}
...
export default function StatusDistributionChart() {
  ...
  const fetch_ = async () => {
    try {
      ...
      if (json.distribution) {
        ...
        const mapped = STATUS_CONFIG.map((s) => ({
          ...s,
          value: json.distribution[s.key] ?? 0,
          total,
        })).filter((s) => s.value > 0);
        setData(mapped);
      }
  ...
  return (
    <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5 h-full shadow-sm">
      ...
      {loading ? (
        <div className="h-[180px] flex items-center justify-center text-stone-300 dark:text-stone-600 text-sm">
          Chargement...
        </div>
      ) : data.length === 0 ? (
        <div className="h-[180px] flex items-center justify-center text-stone-300 dark:text-stone-600 text-sm">
        ...
        {data.map((item) => (
          <div key={item.key} className="flex items-center justify-between text-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="tabular-nums font-semibold text-stone-700 dark:text-stone-200 w-8 text-right">
                {item.value}
              </span>
            </div>
          </div>
