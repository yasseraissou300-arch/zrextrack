...
const STATUS_CONFIG: Record<string, { label: string; badge: string; bg: string; border: string }> =
  {
    ...
    retourne: {
      label: 'Retourné',
      badge: 'bg-stone-100 text-stone-600 dark:text-stone-300',
      bg: 'bg-stone-50',
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        {/* Header statut */}
        <div
          className={`${meta.bg} border-b ${meta.border} px-6 py-4 flex items-start justify-between`}
        >
          <div>
            ...
            <p className="font-mono font-bold text-stone-900 dark:text-stone-100 text-lg">
        ...
        <div className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
                ...
                {STEPS.map((step, idx) => {
                  ...
                  return (
                    <React.Fragment key={step}>
                        ...
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center border-2 text-xs font-bold transition-all ${
                            isDone
                              ...
                              : isActive
                                ? 'bg-white dark:bg-stone-900 border-green-500 text-green-600'
                      ...
                      {idx < STEPS.length - 1 && (
                        <div
                          className={`flex-1 h-0.5 mx-1 mb-4 rounded-full ${idx < stepIdx ? 'bg-green-400' : 'bg-stone-200'}`}
                        />
                      )}
                    </React.Fragment>
                  );
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="bg-stone-50 rounded-xl p-3">
                <p className="text-[10px] text-stone-400 dark:text-stone-500 mb-0.5 flex items-center gap-1">
                  <Phone size={9} />
                  WhatsApp
                </p>
                <p className="text-sm font-semibold text-stone-800 dark:text-stone-100">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="bg-stone-50 rounded-xl p-3">
                ...
                <p className="text-sm font-semibold text-stone-800 dark:text-stone-100">
                  {order.delivery_fees
                    ? `${Number(order.delivery_fees).toLocaleString('fr-DZ')} DA`
                    : '—'}
                </p>
              </div>
              <div className="bg-stone-50 rounded-xl p-3">
          ...
          <div className="flex items-center justify-between border-t border-stone-100 dark:border-stone-800 pt-3">
            <div className="flex items-center gap-1.5 text-xs text-stone-400 dark:text-stone-500">
              <Clock size={11} />
              Mis à jour le{' '}
              <span className="font-medium text-stone-600 dark:text-stone-300 ml-1">
