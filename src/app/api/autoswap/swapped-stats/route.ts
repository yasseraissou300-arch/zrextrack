...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const p of parcels) {
      ...
      const rawState = p.state?.name || p.stateName || p.status?.name || p.statusName || p.state || p.status || '';
      const situation = p.situation?.name || p.situationName || p.lastSituation?.name || p.lastSituationName || p.situation || '';
      const status = mapStatus(String(rawState), String(situation));
