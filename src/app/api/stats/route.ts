...
export async function GET() {
  try {
    ...
    for (const order of allOrders || []) {
      ...
      if (order.status in distribution) distribution[order.status]++;
      ...
      if (order.status === 'livre') daily[key].livrees++;
      else if (order.status === 'echec') daily[key].echecs++;
      else if (order.status === 'retourne') daily[key].retours++;
    }
