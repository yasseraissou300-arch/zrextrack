        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-stone-500 dark:text-stone-400">
              Confirme tes commandes par appel automatique en darija
            </p>
          </div>
        </div>
    ...
    <div className="space-y-4">
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs text-amber-900 space-y-1">
        ...
        <ol className="list-decimal pl-5 space-y-0.5">
          ...
          <li>
            Console Twilio → copie ton <strong>Account SID</strong> et ton{' '}
            <strong>Auth Token</strong> (Dashboard)
          </li>
      ...
      <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5 space-y-3">
        ...
        <Field
          label="Account SID"
          value={s.account_sid || ''}
          onChange={(v) => setS({ ...s, account_sid: v })}
          placeholder="ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
        />
        <Field
          label="Auth Token"
          value={s.auth_token || ''}
          onChange={(v) => setS({ ...s, auth_token: v })}
          placeholder="••••••••"
          mono
        />
        <Field
          label="Numéro Twilio (from)"
          value={s.from_number || ''}
          onChange={(v) => setS({ ...s, from_number: v })}
        ...
        <label className="flex items-center gap-2 pt-2">
          ...
          <span className="text-sm text-stone-700 dark:text-stone-200">
            Activer le module Voice Calling
          </span>
        </label>
      </div>
      ...
      <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5 space-y-3">
        ...
        <Field
          ...
          placeholder="Big Shopping"
        />
        <FieldArea
          label="Message principal (placeholders : {name} {amount} {shop_name})"
          value={s.message_template || ''}
          onChange={(v) => setS({ ...s, message_template: v })}
        />
        <FieldArea
          label="Si touche 1 (confirme)"
          value={s.confirm_text || ''}
          onChange={(v) => setS({ ...s, confirm_text: v })}
        />
        <FieldArea
          label="Si touche 2 (annule)"
          value={s.cancel_text || ''}
          onChange={(v) => setS({ ...s, cancel_text: v })}
        />
        <FieldArea
          label="Si pas de réponse"
          value={s.no_answer_text || ''}
          onChange={(v) => setS({ ...s, no_answer_text: v })}
        />
        <div className="text-[11px] text-stone-500 dark:text-stone-400">
          Voix : <code>Polly.Hala-Neural</code> (féminine arabe, naturelle). Modifiable dans la DB
          plus tard.
        </div>
      </div>
      ...
      <button
        onClick={save}
        disabled={saving}
        className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg disabled:opacity-50"
      >
  ...
  return (
    <div className="space-y-4 max-w-xl">
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 text-xs text-blue-900">
        ...
        <p className="mt-1">
          Avant de lancer des appels en masse, teste sur ton tel personnel pour vérifier la voix, le
          contenu, et que la touche 1/2 fonctionne bien.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5 space-y-3">
        ...
        <Field
          ...
          value={phone}
          onChange={setPhone}
          placeholder="0556172674 ou +213556172674"
        />
        <Field label="Montant DA" value={amount} onChange={setAmount} placeholder="4500" />
        <Field
          label="N° tracking (optionnel)"
          value={tracking}
          onChange={setTracking}
          placeholder="XX-XXXXXXX-ZR"
        />
        <button
          onClick={start}
          disabled={calling || !phone}
          className="w-full flex items-center justify-center gap-2 mt-2 px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl disabled:opacity-50"
        >
    ...
    <div className="space-y-4">
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <Stat
            label="Coût total (DA)"
            value={Math.round(stats.total_cost_da)}
        ...
        {calls.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-stone-50 dark:bg-stone-800 border-b border-stone-100 dark:border-stone-700">
              <tr>
                {['Date', 'Client', 'Téléphone', 'Montant', 'Résultat', 'Durée', 'Coût'].map(
                  (h) => (
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-[11px] font-semibold text-stone-500 dark:text-stone-400 uppercase"
                    >
                      {h}
                    </th>
                  )
            ...
            <tbody className="divide-y divide-stone-50 dark:divide-stone-800">
              {calls.map((c) => (
                <tr key={c.id} className="hover:bg-stone-50 dark:hover:bg-stone-800">
                  ...
                  <td className="px-4 py-3 text-xs font-medium text-stone-800 dark:text-stone-100">
                    {c.customer_name || '—'}
                  </td>
                  <td className="px-4 py-3 text-[11px] font-mono text-stone-500">
                    {c.customer_phone}
                  </td>
                  <td className="px-4 py-3 text-xs text-stone-700 dark:text-stone-200">
                    {c.amount ? `${c.amount} DA` : '—'}
                  </td>
...
function Field({
  ...
  return (
    <div>
      <label className="block text-[11px] font-semibold uppercase tracking-wide text-stone-500 dark:text-stone-400 mb-1">
        {label}
      </label>
      <input
        type="text"
...
function OutcomeBadge({ outcome, status }: { outcome?: string; status?: string }) {
  ...
  if (outcome === 'no_answer' || outcome === 'no_response')
    return (
