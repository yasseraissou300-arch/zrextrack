...
const STATUS_META: Record<
  string,
  { label: string; color: string; bg: string; text: string; icon: LucideIcon }
> = {
  en_preparation: {
    label: 'En préparation',
    color: 'border-purple-400 dark:border-purple-500/40',
  ...
  en_transit: {
    label: 'En transit',
    color: 'border-blue-400 dark:border-blue-500/40',
  ...
  retourne: {
    label: 'Retourné',
    color: 'border-stone-400 dark:border-stone-600',
    bg: 'bg-stone-50 dark:bg-stone-800',
    text: 'text-stone-600 dark:text-stone-300',
    icon: RotateCcw,
  },
};
const getStep = (s: string): number =>
  (
    ({ en_preparation: 0, en_cours: 1, en_transit: 1, en_livraison: 2, livre: 3 }) as Record<
      string,
      number
    >
  )[s] ?? -1;
...
export default function TrackingPage() {
  ...
  const tp =
    typeof params?.tracking === 'string'
      ? params.tracking
      : Array.isArray(params?.tracking)
        ? params.tracking[0]
        : '';
  ...
  const fetch_ = async (t: string) => {
    ...
    try {
      ...
      if (!r.ok || j.error) {
        setError(j.error || 'Commande introuvable');
        setOrder(null);
    ...
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-white to-violet-50 dark:from-stone-950 dark:via-stone-950 dark:to-stone-900">
      ...
      <main className="max-w-2xl mx-auto px-4 py-10 space-y-6">
        ...
        {searched && error && !loading && (
          <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-2xl p-5 text-center">
          ...
          <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 shadow-md overflow-hidden">
            <div className={`${meta.bg} border-b ${meta.color} px-6 py-4 flex items-center gap-3`}>
              ...
              <div>
                ...
                <p className={`font-bold text-lg ${meta.text}`}>{meta.label}</p>
              </div>
              <div className="ml-auto text-right">
                <p className="text-xs text-stone-400 dark:text-stone-500">Tracking</p>
                <p className="font-mono font-bold text-stone-800 dark:text-stone-100 text-sm">
                  {order.tracking}
                </p>
              </div>
            </div>
              ...
              <div className="px-6 pt-5 pb-2">
                <div className="flex items-center">
                  {STATUS_STEPS.map((step, idx) => {
                    const isA = idx === stepIdx,
                      isDone = idx < stepIdx,
                      Icon = step.icon;
                    return (
                      <div key={step.key} className="flex items-center flex-1 last:flex-none">
                        <div
                          className={`flex flex-col items-center gap-1 ${idx <= stepIdx ? '' : 'opacity-30'}`}
                        >
                          <div
                            className={`w-9 h-9 rounded-full flex items-center justify-center border-2 ${isA ? `${step.bg} ${step.border}` : isDone ? 'bg-green-100 dark:bg-green-500/20 border-green-400 dark:border-green-500/50' : 'bg-stone-50 dark:bg-stone-800 border-stone-200 dark:border-stone-700'}`}
                          >
                            {isDone ? (
                              <CheckCircle2
                                size={16}
                              ...
                              <Icon
                                size={16}
                                className={isA ? step.color : 'text-stone-400 dark:text-stone-500'}
                              />
                            )}
                          </div>
                          <span
                            className={`text-[10px] font-medium text-center ${isA ? step.color : isDone ? 'text-green-600 dark:text-green-400' : 'text-stone-400 dark:text-stone-500'}`}
                          >
                            {step.label}
                          </span>
                        </div>
                        {idx < STATUS_STEPS.length - 1 && (
                          <div
                            className={`flex-1 h-0.5 mx-1 mb-5 rounded-full ${idx < stepIdx ? 'bg-green-400 dark:bg-green-500/50' : 'bg-stone-200 dark:bg-stone-700'}`}
                          />
                        )}
                      </div>
                    );
            ...
            <div className="grid grid-cols-2 gap-3 px-6 py-4">
              {order.client && (
                <div className="bg-stone-50 dark:bg-stone-800/60 rounded-xl p-3">
                  <p className="text-[10px] text-stone-400 dark:text-stone-500 uppercase mb-0.5">
                    Client
                  </p>
              ...
              {order.wilaya && (
                <div className="bg-stone-50 dark:bg-stone-800/60 rounded-xl p-3">
                  <p className="text-[10px] text-stone-400 dark:text-stone-500 uppercase mb-0.5 flex items-center gap-1">
                    <MapPin size={9} />
                    Wilaya
                  </p>
                  <p className="font-semibold text-stone-800 dark:text-stone-100 text-sm">
              ...
              <div className="px-6 pb-4 flex items-center gap-1.5">
                ...
                <p className="text-xs text-stone-400 dark:text-stone-500">
                  ...
                  <span className="font-medium text-stone-600 dark:text-stone-300">
                    {new Date(order.last_update).toLocaleString('fr-FR', {
                      day: '2-digit',
                      month: 'long',
                      year: 'numeric',
                      hour: '2-digit',
                ...
                <button
                  onClick={() => fetch_(order.tracking)}
                  className="ml-auto flex items-center gap-1 text-xs text-violet-600 dark:text-violet-400 font-medium"
                >
                  <RefreshCw size={11} />
                  Actualiser
                </button>
            ...
            {order.status === 'echec' && (
              <div className="mx-6 mb-4 bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/30 rounded-xl p-3 text-xs text-red-700 dark:text-red-300">
                ⚠️ Notre livreur n'a pas pu vous joindre. Contactez le vendeur.
              </div>
            )}
            {order.status === 'retourne' && (
              <div className="mx-6 mb-4 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-xl p-3 text-xs text-stone-600 dark:text-stone-300">
                📦 Colis retourné. Contactez le vendeur.
              </div>
            )}
            {order.status === 'livre' && (
              <div className="mx-6 mb-4 bg-green-50 dark:bg-green-500/10 border border-green-100 dark:border-green-500/30 rounded-xl p-3 text-xs text-green-700 dark:text-green-300 text-center">
                🎉 Livré avec succès. Merci pour votre confiance !
              </div>
            )}
          </div>
        )}
