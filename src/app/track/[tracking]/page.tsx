...
const STATUS_META = {
  ...
  retourne: { label: 'Retourné', color: 'border-stone-400', bg: 'bg-stone-50', text: 'text-stone-600', icon: RotateCcw },
};
      ...
      <header className="bg-white border-b border-stone-100 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/track" className="flex items-center gap-2">
            ...
            <span className="font-bold text-stone-900">Autotim</span>
          </Link>
          <span className="text-xs text-stone-400">Suivi de commande</span>
        </div>
      </header>
      <main className="max-w-2xl mx-auto px-4 py-10 space-y-6">
        <div className="text-center"><h1 className="text-2xl font-bold text-stone-900 mb-1">Suivre ma commande</h1></div>
        <div className="flex gap-2">
          <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key==='Enter' && fetch_(input)} placeholder="Ex : ZRX123456"
            className="flex-1 border border-stone-200 rounded-xl px-4 py-3 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-green-400 shadow-sm" />
        ...
        {order && !loading && meta && (
          <div className="bg-white rounded-2xl border border-stone-100 shadow-md overflow-hidden">
            <div className={`${meta.bg} border-b ${meta.color} px-6 py-4 flex items-center gap-3`}>
              ...
              <div><p className="text-xs text-stone-500 uppercase tracking-wide">Statut actuel</p><p className={`font-bold text-lg ${meta.text}`}>{meta.label}</p></div>
              <div className="ml-auto text-right"><p className="text-xs text-stone-400">Tracking</p><p className="font-mono font-bold text-stone-800 text-sm">{order.tracking}</p></div>
            </div>
            {!isTerminal && (
                ...
                {STATUS_STEPS.map((step, idx) => { const isA=idx===stepIdx,isDone=idx<stepIdx,Icon=step.icon; return (
                  <div key={step.key} className="flex items-center flex-1 last:flex-none">
                    <div className={`flex flex-col items-center gap-1 ${idx<=stepIdx?'':'opacity-30'}`}>
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 ${isA?`${step.bg} ${step.border}`:isDone?'bg-green-100 border-green-400':'bg-stone-50 border-stone-200'}`}>
                        {isDone?<CheckCircle2 size={16} className="text-green-500"/>:<Icon size={16} className={isA?step.color:'text-stone-400'}/>}
                      </div>
                      <span className={`text-[10px] font-medium text-center ${isA?step.color:isDone?'text-green-600':'text-stone-400'}`}>{step.label}</span>
                    </div>
                    {idx<STATUS_STEPS.length-1 && <div className={`flex-1 h-0.5 mx-1 mb-5 rounded-full ${idx<stepIdx?'bg-green-400':'bg-stone-200'}`}/>}
                  </div>); })}
              </div></div>
            )}
            <div className="grid grid-cols-2 gap-3 px-6 py-4">
              {order.client && <div className="bg-stone-50 rounded-xl p-3"><p className="text-[10px] text-stone-400 uppercase mb-0.5">Client</p><p className="font-semibold text-stone-800 text-sm">{order.client}</p></div>}
              {order.wilaya && <div className="bg-stone-50 rounded-xl p-3"><p className="text-[10px] text-stone-400 uppercase mb-0.5 flex items-center gap-1"><MapPin size={9}/>Wilaya</p><p className="font-semibold text-stone-800 text-sm">{order.wilaya}</p></div>}
              {order.product && <div className="bg-stone-50 rounded-xl p-3"><p className="text-[10px] text-stone-400 uppercase mb-0.5">Produit</p><p className="font-semibold text-stone-800 text-sm">{order.product}</p></div>}
              {order.attempts!=null && <div className="bg-stone-50 rounded-xl p-3"><p className="text-[10px] text-stone-400 uppercase mb-0.5">Tentatives</p><p className="font-semibold text-stone-800 text-sm">{order.attempts}</p></div>}
            </div>
            {order.last_update && <div className="px-6 pb-4 flex items-center gap-1.5">
              <Clock size={12} className="text-stone-400"/>
              <p className="text-xs text-stone-400">Mis à jour le <span className="font-medium text-stone-600">{new Date(order.last_update).toLocaleString('fr-FR',{day:'2-digit',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit'})}</span></p>
            ...
            {order.status==='retourne' && <div className="mx-6 mb-4 bg-stone-50 border border-stone-200 rounded-xl p-3 text-xs text-stone-600">📦 Colis retourné. Contactez le vendeur.</div>}
