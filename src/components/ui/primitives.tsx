...
const STATUS: Record<string, { label: string; cls: string }> = {
  en_preparation: { label: 'En préparation', cls: 'bg-purple-100 text-purple-700 dark:bg-purple-500/15 dark:text-purple-300' },
  en_transit:     { label: 'En transit',     cls: 'bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300' },
  en_livraison:   { label: 'En livraison',   cls: 'bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300' },
  livre:          { label: 'Livré',          cls: 'bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-300' },
  echec:          { label: 'Échec',          cls: 'bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-300' },
  retourne:       { label: 'Retourné',       cls: 'bg-stone-100 text-stone-600 dark:bg-stone-700/50 dark:text-stone-300' },
};
...
export function StatusBadge({ status, className = '' }: { status: string; className?: string }) {
  const s = STATUS[status] ?? { label: status || '—', cls: 'bg-stone-100 text-stone-600 dark:bg-stone-800 dark:text-stone-400' };
  return (
    <span className={`inline-flex items-center text-xs font-medium px-2.5 py-1 rounded-full ${s.cls} ${className}`}>
      {s.label}
    </span>
  );
}
...
export function PageHeader({
  icon: Icon, title, subtitle, tone = 'violet', action,
}: {
  icon: LucideIcon; title: string; subtitle?: string; tone?: Tone; action?: React.ReactNode;
}) {
  return (
    <div className="flex items-center justify-between gap-3 flex-wrap">
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${TONE_CLS[tone]}`}>
          <Icon size={20} />
        </div>
        <div>
...
export function StatCard({
  icon: Icon, label, value, tone = 'violet', loading = false,
}: {
  icon: LucideIcon; label: string; value: React.ReactNode; tone?: Tone; loading?: boolean;
}) {
  return (
    <div className="bg-white dark:bg-stone-900 rounded-2xl p-4 shadow-sm border border-stone-100 dark:border-stone-800">
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center mb-2.5 ${TONE_CLS[tone]}`}>
        <Icon size={16} />
      </div>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      ...
      {subtitle && <p className="text-sm text-stone-400 dark:text-stone-500 mt-1 max-w-sm">{subtitle}</p>}
