...
const UPGRADE_MAILTO =
  'mailto:yasseraissou300@gmail.com?subject=Autotim%20%E2%80%94%20Upgrade%20de%20plan&body=Bonjour%20Yasser%2C%0A%0AJe%20souhaite%20passer%20mon%20plan%20Autotim%20%C3%A0%20un%20niveau%20sup%C3%A9rieur%20(mon%20quota%20est%20atteint).%0A%0AMerci.';

export default function QuotaBanner() {
  const [state, setState] = useState<QuotaState | null>(null);

  useEffect(() => {
    fetch('/api/quotas/state')
      .then((r) => r.json())
      .then(setState)
      .catch(() => setState(null));
  }, []);
    ...
    return (
      <div className="flex items-center gap-3 text-xs text-stone-500 dark:text-stone-400 px-3">
        <span className="tabular-nums">
          Ce mois : <strong className="text-stone-700 dark:text-stone-200">{state.used}</strong> /{' '}
          {state.quota} commandes ({state.planLabel})
        </span>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-stone-900 dark:text-stone-100">
            {isOver ? 'Quota mensuel atteint' : `Plus que ${state.remaining} commandes ce mois`}
          </h3>
          <p className="text-sm text-stone-600 dark:text-stone-300 mt-0.5">
            {isOver ? (
              <>
                Votre plan {state.planLabel} est limité à {state.quota} commandes/mois. Les
                prochaines syncs seront bloquées jusqu'au 1er du mois prochain.
              </>
            ) : (
              <>
                Vous êtes à {state.percent}% de votre quota mensuel {state.planLabel}. Pensez à
                passer au plan supérieur pour éviter le blocage.
              </>
            )}
          </p>
