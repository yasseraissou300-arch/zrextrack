...
import {
  isSituationSwappable, isSwappable, isTarget,
  normalizeParcel, matchSwappables, parseProductsDescription,
...
describe('isSwappable — flag API + repli situation + limite de 2 swaps', () => {
  it('accepte quand l\'API positionne isEligibleForSwap', () => {
    const p = normalizeParcel(colis({ swap: { count: 0, isEligibleForSwap: true } } as Partial<ZRParcel>));
  ...
  it('accepte encore à count = 1', () => {
    const p = normalizeParcel(colis({
      situation: 'Ne répond pas 3', swap: { count: 1, isEligibleForSwap: false },
    } as Partial<ZRParcel>));
    expect(isSwappable(p)).toBe(true);
  });
});
...
describe('isTarget — commandes en attente (comparaison normalisée)', () => {
  it('reconnaît snake_case ET libellé accentué', () => {
    ...
    for (const s of ['commande_recue', 'Commande reçue', 'pret_a_expedier', 'Prêt à expédier']) {
      expect(isTarget(normalizeParcel(colis({ state: { name: s } } as Partial<ZRParcel>)))).toBe(true);
    }
  });

  it('exclut les états déjà expédiés', () => {
    for (const s of ['Vers Wilaya', 'Sortie en livraison', 'Livré', 'Dispatch']) {
      expect(isTarget(normalizeParcel(colis({ state: { name: s } } as Partial<ZRParcel>)))).toBe(false);
    }
  });
});
...
describe('matchSwappables — appariement', () => {
  const src = () => colis({
    id: 'SRC', trackingNumber: 'SRC-1', situation: 'Ne répond pas 3',
  ...
  it('apparie un produit identique (STRONG)', () => {
    const p = matchSwappables([
      src(),
      colis({ id: 'TGT', trackingNumber: 'TGT-1', state: { name: 'commande_recue' },
        desc: 'pantalon lain( pl )( noir M ) :  - 1' } as Partial<ZRParcel>),
    ]);
  ...
  it('apparie partiellement un pack multi-couleurs (WEAK)', () => {
    ...
    const p = matchSwappables([
      colis({ id: 'S', trackingNumber: 'S-1', situation: 'Commande annulée',
        desc: 'pantalon lain( pl )( noir , bleu M ) :  - 1' } as Partial<ZRParcel>),
      colis({ id: 'T', trackingNumber: 'T-1', state: { name: 'commande_recue' },
        desc: 'pantalon lain( pl )( noir M ) :  - 1' } as Partial<ZRParcel>),
    ]);
    expect(p).toHaveLength(1);
  ...
  it('REFUSE des produits différents', () => {
    expect(matchSwappables([
      src(),
      colis({ id: 'T', state: { name: 'commande_recue' },
        desc: 'robe ete( re )( noir M ) :  - 1' } as Partial<ZRParcel>),
    ])).toHaveLength(0);
  });
  ...
  it('REFUSE si aucune couleur commune', () => {
    expect(matchSwappables([
      src(),
      colis({ id: 'T', state: { name: 'commande_recue' },
        desc: 'pantalon lain( pl )( rouge M ) :  - 1' } as Partial<ZRParcel>),
    ])).toHaveLength(0);
  });
  ...
  it('REFUSE si les quantités diffèrent', () => {
    expect(matchSwappables([
      src(),
      colis({ id: 'T', state: { name: 'commande_recue' },
        desc: 'pantalon lain( pl )( noir M ) :  - 3' } as Partial<ZRParcel>),
    ])).toHaveLength(0);
  });
  ...
  it('RESPECTE la restriction des wilayas du Sud (Adrar = 1)', () => {
    ...
    const sud = colis({
      id: 'S', situation: 'Ne répond pas 3', desc: 'pantalon lain( pl )( noir M ) :  - 1',
      deliveryAddress: { city: 'Adrar', cityTerritoryId: 'adr', cityTerritoryCode: 1 },
    } as Partial<ZRParcel>);
    const alger = colis({
      id: 'T', state: { name: 'commande_recue' }, desc: 'pantalon lain( pl )( noir M ) :  - 1',
      deliveryAddress: { city: 'Alger', cityTerritoryId: 'alg', cityTerritoryCode: 16 },
    } as Partial<ZRParcel>);
    expect(matchSwappables([sud, alger])).toHaveLength(0);

    const adrar2 = colis({
      id: 'T2', state: { name: 'commande_recue' }, desc: 'pantalon lain( pl )( noir M ) :  - 1',
      deliveryAddress: { city: 'Adrar', cityTerritoryId: 'adr', cityTerritoryCode: 1 },
    } as Partial<ZRParcel>);
  ...
  it('n\'utilise JAMAIS deux fois le même colis (affectation 1-pour-1)', () => {
    const props = matchSwappables([
      src(),
      colis({ id: 'T1', trackingNumber: 'T-1', state: { name: 'commande_recue' },
        desc: 'pantalon lain( pl )( noir M ) :  - 1' } as Partial<ZRParcel>),
      colis({ id: 'T2', trackingNumber: 'T-2', state: { name: 'commande_recue' },
        desc: 'pantalon lain( pl )( noir M ) :  - 1' } as Partial<ZRParcel>),
    ]);
